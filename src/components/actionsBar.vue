<template>
  <div class="actionsContainer">
    ...
    <img
      ...
      class="editBts"
      id="editBt"
      @click.stop="showListAddBox"
      alt=""
    />
    <img
      src="@/assets/back.svg"
      v-on:click.stop="collapseNote"
      class="colapse"
      alt=""
    />
  </div>
...
export default {
  methods: {
    showListAddBox() {
      if (document.body.classList.contains("view=Shopping")) {
        ...
        document
          ...
          .classList.remove("expandedList");
        document.body.classList.remove("listExpanded");
        document
          .querySelector(".actions-visible")
          .classList.remove("actions-visible");
    ...
    confirmDelete() {
      if (document.body.classList.contains("view=Shopping")) {
        document.body.classList.add("showConfirmBox");
  ...
  created() {
    setTimeout(() => {
      if (document.body.classList.contains("view=Notes")) {
        document.querySelectorAll(".editBts").forEach((bt) => {
          bt.style.opacity = "0";
        });
      }
    }, 0);
...
@media (min-width: 600px) {
  .actionsContainer {
    ...
    img:hover {
      transform: scale(1.1);
      box-shadow: 0px 2px 20px rgba(0, 0, 0, 0.178);
    }
    #editBt {
      width: 50px;
    }
