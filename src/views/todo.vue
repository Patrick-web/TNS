<template>
  <div class="page mainTodoContainer">
    <titleBar title="Tasks" />
    {{ datetime }}
    <div accept-charset="utf-8" class="formArea todoForm">
      ...
      <input
        ...
        value=""
      />
      <img id="pickerToggle" src="@/assets/watch.svg" alt="" />
      <datetime
...
export default {
  ...
  methods: {
    ...
    addTask() {
      ...
      let dueTime = null;
      if (this.datetime) {
        const day = new Date.parse(this.datetime);
        dueDate = day.toString("d-MMM-yyyy");
        dueTime = day.toString("HH:mm");
      }
      const newTask = {
        id: Date.now(),
        task: task.value,
    ...
    deleteSubTask(subIndex, e) {
      const mainTaskIndex = e.target.parentElement.parentElement.parentElement.parentElement.parentElement.querySelector(
        ".taskIndex"
      ).textContent;
    ...
    saveSubtaskEdit() {
      if (this.getInput()) {
        this.tasks[this.targetMaintask].subtasks[
          this.targetSubtask
        ].task = this.getInput();
        this.persistToStorage();
        this.removeFocus();
    ...
    markSubAsDone(subIndex, e) {
      ...
      this.tasks[mainTaskindex].subtasks[subIndex].isDone = !this.tasks[
        mainTaskindex
      ].subtasks[subIndex].isDone;
